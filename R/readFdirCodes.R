...
#' @return A named list with two elements: \code{$speciesCodes} (data frame with columns \code{idNS}, \code{idFAO}, \code{norwegian}, \code{english}, \code{latin}) and \code{$gearCodes} (data frame with columns \code{idGear}, \code{gearName}, \code{gearCategory}, \code{Hovedgruppe}, \code{Subgruppe}).
