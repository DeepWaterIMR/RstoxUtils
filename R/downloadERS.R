...
#' @return Returns a data frame showing which files were modified. Downloads data into folder specified by the \code{dest} argument.
#' @family ERS functions
#' @author Mikko Vihtakari
#' @export

# years = 2011:as.integer(format(Sys.Date(), "%Y")); dest = '/Users/a22357/ownCloud/Workstuff/Data/Norwegian Electronic Logbooks/Data/ERS data'; overwrite = "yes"; fdir_url = "https://register.fiskeridir.no/vms-ers/ERS/"
  ...
  out <- lapply(years, function(k) {
    ...
    if(overwrite == "force") {
      ...
      return({
        data.frame(
          year = k,
          last_modified_fdir_url = last_mod_fdir,
          last_modified_dest = last_mod_disk,
          updated = TRUE,
          file_dir = file_dir)
      })
